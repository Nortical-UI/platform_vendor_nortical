...
ifeq ($(TARGET_BUILD_VARIANT),OFFICIAL)
     ...
     PROD_VERSION += Nortical-$(PLATFORM_NORTICAL_VERSION)-$(TARGET_DEVICE)-$(shell date +%m%d%H%M)-OFFICIAL
 else
     PLATFORM_NORTICAL_VERSION := $(PLATFORM_NORTICAL_RELEASE)
     PROD_VERSION += Nortical-$(PLATFORM_NORTICAL_VERSION)-$(TARGET_DEVICE)-$(shell date +%m%d%H%M)-UNOFFICIAL
 endif
