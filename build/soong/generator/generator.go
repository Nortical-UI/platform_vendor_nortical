...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = norticalExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = norticalExpandVariables(ctx, dep_file)
	...
	exCmd := norticalExpandVariables(ctx, String(g.properties.Cmd))
	...
	manifestName := "nortical.sbox.textproto"
	desc := "generate"
	name := "generator"
